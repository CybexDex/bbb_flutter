import 'package:bbb_flutter/vendor/path_drawing/src/dash_path.dart';
...
//    final int gridRows = 4, gridColumns = 4;
    ...
    for (int i = 0; i <= gridRows; i++) {
      if (i != gridRows) {
        canvas.drawPath(
            dashPath(
                Path()
                  ..moveTo(4, rowSpace * i + topPadding)
    ...
    for (int i = 0; i <= columnSpace; i++) {
      canvas.drawPath(
          dashPath(
              Path()
                ..moveTo(columnSpace * i, topPadding / 3)
                ..lineTo(columnSpace * i, chartRect.bottom),
              dashArray: CircularIntervalList<double>(<double>[4, 4])),
          gridPaint);
