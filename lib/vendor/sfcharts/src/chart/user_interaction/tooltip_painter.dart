...
class _TooltipPainter extends CustomPainter {
        ...
        for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
          ...
          if (series._visible &&
            ...
            series._regionalData.forEach((dynamic regionRect, dynamic values) {
              String strike;
              String username;
              if (series.dataSource[counter].order != null){
                strike = series.dataSource[counter].order.strike.toString();
                username = series.dataSource[counter].order.username;
              }
              counter = counter + 1;
              ...
              if (paddedRegion.contains(position)) {
                ...
                header = (header == null)
                    ...
                    : header;
                chart.tooltipBehavior._painter.header = username??header;
                if (chart.tooltipBehavior.format != null) {
                  final String resultantString = chart.tooltipBehavior.format
                      ...
                      .replaceAll('series.name', "hello" ?? 'hello')
  ...
  void renderTooltip(Canvas canvas) {
    ...
    if (x != null &&
      ...
      tooltipArgs.seriesIndex = _chart is SfCartesianChart
          ? currentSeries.segments[tooltipArgs.pointIndex]._seriesIndex
          : currentSeries;
    ...
    if (x != null &&
      ...
      if (y > pointerLength + rect.height && y > boundaryRect.top) {
        if (_chart is SfCartesianChart) {
          if (currentSeries._seriesType == 'bubble') padding = 2;
        }
        isTop = true;
        xPos = x - (rect.width / 2);
      ...
      if (header != null && header.isNotEmpty) {
        ...
        canvas.drawLine(
            Offset(tooltipRect.left + padding,
                tooltipRect.bottom - tooltipRect.height / 2),
  ...
  void _drawTooltipText(dynamic tooltip, Canvas canvas, String text,
      Offset point, ChartTextStyle style,
      [int rotation]) {
    TextAlign tooltipTextAlign = TextAlign.center;
    if (tooltip != null &&
        tooltip.format != null &&
