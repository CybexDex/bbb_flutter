      ...
      for (int yAxisIndex = 0;
        ...
        for (int ySeriesIndex = 0;
          ...
          if (xAxisSeries == yAxisSeries &&
              (xAxisSeries is ColumnSeries || xAxisSeries is BarSeries) &&
...
dynamic _getLabelValue(dynamic value, dynamic axis) {
  if (value is double) {
    final String str = value.toString();
    final List<dynamic> list = str.split('.');
    value = double.parse(value.toStringAsFixed(2));
    value = (list[1] == '0' || list[1] == '00') ? value.round() : value;
  }
...
dynamic _getCrosshairLabel(dynamic value, ChartAxis axis) {
  if (axis is CategoryAxis) {
    value = value < 0 ? 0 : value;
    value = axis._visibleLabels[value.round()].text;
    ...
    value = _getLabelValue(value, axis);
  }
  return value;
}
      ...
      if (xAxisName == axis._name ||
        ...
        if (xAxis is CategoryAxis) {
          if (xAxis._labels.length > 0) {
            xValue = xAxis._labels.indexOf(annotation.x.toString());
          }
