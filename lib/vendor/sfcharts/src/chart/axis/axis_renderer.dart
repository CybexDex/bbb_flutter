...
class _ChartAxisRenderer with _CustomizeAxisElements {
    ...
    if (length > 0) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (axis.majorGridLines.width > 0 &&
            renderType == 'outside' &&
            (axis.plotOffset > 0 ||
                ((i == 0 || i == length - 1) && chart.borderWidth == 0) ||
                (((i == 0 && !axis.opposedPosition) ||
                        (i == length - 1 && axis.opposedPosition)) &&
                    axis.axisLine.width == 0) ||
                (axisBounds.left < pointX - axis.majorGridLines.width &&
                    axisBounds.right > pointX + axis.majorGridLines.width))) {
      ...
      if (axis.minorGridLines.width > 0 && renderType == 'outside') {
        _drawDashedPath(
            ...
            _CustomPaintStyle(
                ...
                PaintingStyle.stroke),
            Offset(position, rect.top),
            Offset(
                position,
                !axis.opposedPosition
                    ? pointY - chart._chartAxis._axisClipRect.height
                    : pointY + chart._chartAxis._axisClipRect.height));
      }
        ...
        if ((value < range.maximum) && (value > range.minimum)) {
          ...
          if (axis.minorGridLines.width > 0 &&
            ...
            _drawDashedPath(
                ...
                Offset(
                    !axis.opposedPosition
                        ? rect.left + chart._chartAxis._axisClipRect.width
                        : rect.left - chart._chartAxis._axisClipRect.width,
                    position));
          }
          if (axis.minorTickLines.width > 0 &&
  ...
  void drawHorizontalAxesLabels(
    ...
    if (renderType == axis.labelPosition.toString().split('.')[1]) {
      ...
      for (int i = 0; i < visibleLabels.length; i++) {
        ...
        String labelText = axis._axisRenderer.getAxisLabel(axis, label.text, i);
        textStyle = axis._axisRenderer.getAxisLabelStyle(axis, labelText, i);
        ...
        if (chart.onAxisLabelRender != null) {
          ...
          axisLabelArgs.value = label.value;
          axisLabelArgs.textStyle = textStyle;
          axisLabelArgs.axis = axis;
          axisLabelArgs.orientation = axis._orientation;
          axisLabelArgs.axisName = axis._name;
          chart.onAxisLabelRender(axisLabelArgs);
          labelText = axisLabelArgs.text;
          label.value = axisLabelArgs.value;
        ...
        pointX = (angle == 45) ? location.x + textSize.width / 2 : location.x;
        pointY = location.y;
    ...
    if (axis.labelPosition.toString().split('.')[1] == renderType) {
      ...
      for (dynamic i = 0; i < visibleLabels.length; i++) {
        ...
        if (chart.onAxisLabelRender != null) {
          axisLabelArgs = AxisLabelRenderArgs();
          axisLabelArgs.text = labelText;
          axisLabelArgs.value = tempInterval;
          axisLabelArgs.textStyle = textStyle;
          axisLabelArgs.axis = axis;
          axisLabelArgs.orientation = axis._orientation;
          axisLabelArgs.axisName = axis._name;
          chart.onAxisLabelRender(axisLabelArgs);
          labelText = axisLabelArgs.text;
          tempInterval = axisLabelArgs.value;
          textStyle = axisLabelArgs.textStyle;
        }
  ...
  void drawHorizontalAxesTitle(
    ...
    axis.title.alignment == ChartAlignment.near
        ? point = Offset(axisBounds.left, top)
        : axis.title.alignment == ChartAlignment.far
            ? point = Offset(axisBounds.right - textSize.width, top)
            : point = Offset(
                axisBounds.left +
  ...
  void drawVerticalAxesTitle(
    ...
    axis.title.alignment == ChartAlignment.near
        ? point = Offset(
            left,
            axis.opposedPosition
                ? axisBounds.bottom - textSize.width
                : axisBounds.bottom)
        : axis.title.alignment == ChartAlignment.far
            ? point = Offset(
                left,
...
class _CartesianAxesPainter extends CustomPainter {
  ...
  void _drawBackgroundImage(Canvas canvas) {
    ...
    void _loadImage() async {
      ...
      if (!_drawImage) chart._chartState.axisRepaintNotifier.value++;
      _drawImage = true;
    }
