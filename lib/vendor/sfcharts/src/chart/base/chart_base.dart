...
class _SfCartesianChartState extends State<SfCartesianChart>
  ...
  void initializeDefaultValues() {
    ...
    containerRect = Rect.fromLTRB(0, 0, 0, 0);
    zoomProgress = false;
    initialRender = true;
    ...
    for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
      ...
      _chartWidgets.add(Container(
        ...
        painter: _getSeriesPainter(i, chart._chartState.animationController,
            seriesAnimation, chartElementAnimation),
      ))));
    ...
    CartesianSeries<dynamic, dynamic> _findSeries(Offset position) {
      ...
      for (int i = chart._chartSeries.visibleSeries.length - 1; i >= 0; i--) {
        ...
        if (series._seriesType == 'column' ||
          ...
          for (int j = 0; j < series._dataPoints.length; j++) {
            if (series._dataPoints[j].region.contains(position)) {
              break outerLoop;
            }
          }
        } else {
          ...
          for (int k = chart._chartSeries.visibleSeries.length - 1;
              k >= 0;
              k--) {
            isSelect = series.selectionSettings._selectionRenderer
    ...
    _chartWidgets.add(Listener(
        onPointerDown: (PointerDownEvent event) {
          ...
          if (chart.zoomPanBehavior.enablePinching == true) {
            ...
            if (chart.zoomPanBehavior._touchStartPositions.length < 2) {
              chart.zoomPanBehavior._touchStartPositions.add(event);
            }
            if (chart.zoomPanBehavior._touchStartPositions.length == 2) {
              for (int axisIndex = 0;
                ...
                chart.zoomPanBehavior.onPinchStart(
                    axis,
                    chart.zoomPanBehavior._touchStartPositions[0].position.dx,
                    chart.zoomPanBehavior._touchStartPositions[0].position.dy,
                    chart.zoomPanBehavior._touchStartPositions[1].position.dx,
                    chart.zoomPanBehavior._touchStartPositions[1].position.dy,
                    axis._zoomFactor);
              }
            }
          }
        ...
        onPointerMove: (PointerMoveEvent event) {
          ...
          if (chart.zoomPanBehavior.enablePinching == true &&
              chart.zoomPanBehavior._touchStartPositions.length == 2) {
            chart.zoomPanBehavior._isPinching = true;
            final int pointerID = event.pointer;
            bool addPointer = true;
            for (int i = 0;
                i < chart.zoomPanBehavior._touchMovePositions.length;
                i++) {
              if (chart.zoomPanBehavior._touchMovePositions[i].pointer ==
                  pointerID) {
                addPointer = false;
              }
            }
            if (chart.zoomPanBehavior._touchMovePositions.length < 2 &&
                addPointer) {
              chart.zoomPanBehavior._touchMovePositions.add(event);
            }

            if (chart.zoomPanBehavior._touchMovePositions.length == 2 &&
                chart.zoomPanBehavior._touchStartPositions.length == 2) {
              if (chart.zoomPanBehavior._touchMovePositions[0].pointer ==
                  event.pointer) {
                chart.zoomPanBehavior._touchMovePositions[0] = event;
              }
              if (chart.zoomPanBehavior._touchMovePositions[1].pointer ==
                  event.pointer) {
                chart.zoomPanBehavior._touchMovePositions[1] = event;
              }
              chart.zoomPanBehavior._performPinchZooming(
                  chart.zoomPanBehavior._touchStartPositions,
                  chart.zoomPanBehavior._touchMovePositions);
            }
          }
        },
        onPointerUp: (PointerUpEvent event) {
          ...
          int axisIndex;
          if (chart.zoomPanBehavior._touchStartPositions.length == 2 &&
              chart.zoomPanBehavior._touchMovePositions.length == 2 &&
            ...
            for (axisIndex = 0;
              ...
              chart.zoomPanBehavior.onPinchEnd(
                  axis,
                  chart.zoomPanBehavior._touchMovePositions[0].position.dx,
                  chart.zoomPanBehavior._touchMovePositions[0].position.dy,
                  chart.zoomPanBehavior._touchMovePositions[1].position.dx,
                  chart.zoomPanBehavior._touchMovePositions[1].position.dy,
                  axis._zoomFactor);
            }
            if (resetFlag) {
              for (int index = 0;
          ...
          chart.zoomPanBehavior._touchStartPositions = <PointerEvent>[];
          chart.zoomPanBehavior._touchMovePositions = <PointerEvent>[];
  ...
  void _getPointSeriesIndex(SfCartesianChart chart, Offset position) {
    for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
      ...
      for (int k = 0; k < series.segments.length; k++) {
        if (series.segments[k].segmentRect.contains(position)) {
          PointTapArgs pointTapArgs;
          pointTapArgs = PointTapArgs();
          pointTapArgs.pointIndex = k;
