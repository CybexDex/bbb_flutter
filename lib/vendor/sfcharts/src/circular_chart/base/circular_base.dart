...
typedef CircularLegendRenderCallback = LegendRenderArgs Function(
    LegendRenderArgs legendRenderArgs);

/// Returns the TooltipArgs.
typedef CircularTooltipCallback = TooltipArgs Function(TooltipArgs tooltipArgs);

/// Returns the DataLabelRenderArgs.
typedef CircularDatalabelRenderCallback = DataLabelRenderArgs Function(
    DataLabelRenderArgs dataLabelArgs);

/// Returns the PointTapArgs.
typedef CircularPointTapCallback = PointTapArgs Function(
...
class _SfCircularChartState extends State<SfCircularChart>
  ...
  void didUpdateWidget(SfCircularChart oldWidget) {
    ...
    oldWidget._chartState.animateCompleted = false;
...
class _CircularArea extends StatelessWidget {
  ...
  void _onTapDown(BuildContext context, Offset globalPosition) {
    ...
    if (chart._chartState._tapPosition != null &&
      ...
      chart._chartState.currentActive = _ChartInteraction(
          ...
          pointRegion);
      series.selectionSettings._selectionRenderer.series = series;
      series.selectionSettings
          .onCircularTouchDown(pointRegion.pointIndex, pointRegion.seriesIndex);
    } else {
  ...
  void onDoubleTap() {
    if (doubleTapPosition != null &&
      ...
      series.selectionSettings._selectionRenderer.series = series;
      series.selectionSettings
          .onCircularDoubleTap(pointRegion.pointIndex, pointRegion.seriesIndex);
  ...
  void onLongPress() {
    if (chart._chartState._tapPosition != null &&
      ...
      series.selectionSettings._selectionRenderer.series = series;
      series.selectionSettings
          .onCircularLongPress(pointRegion.pointIndex, pointRegion.seriesIndex);
      if (chart._chartState.currentActive != null) {
        if (chart._chartState.currentActive.series.explodeGesture ==
  ...
  void _bindSeriesWidgets() {
    ...
    for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
      ...
      if (series.selectionSettings != null) {
        series.selectionSettings._selectionRenderer ??= _SelectionRenderer();
        series.selectionSettings._selectionRenderer.chart = chart;
        series.selectionSettings._selectionRenderer.selectedDataPoints =
            chart._chartState.selectedDataPoints;
        series.selectionSettings._selectionRenderer.unselectedDataPoints =
            chart._chartState.unselectedDataPoints;
        series.selectionSettings._selectionRenderer.selectedRegions =
            chart._chartState.selectedRegions;
        series.selectionSettings._selectionRenderer.unselectedRegions =
            chart._chartState.unselectedRegions;
      ...
      if (series.animationDuration > 0 && chart._chartState.initialRender) {
