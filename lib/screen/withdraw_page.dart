    ...
    return GestureDetector(
      ...
      child: BaseWidget<WithdrawViewModel>(
        model: withdrawViewModel = WithdrawViewModel(
            api: locator.get(), refm: locator.get(), um: locator.get(), gatewayApi: locator.get()),
        onModelReady: (model) {
          model.initForm();
                  ...
                  children: <Widget>[
                      ...
                      children: <Widget>[
                        Row(
                          children: <Widget>[
                            ...
                            GestureDetector(
                              child: Text("全部", style: StyleFactory.errorMessageText),
                              onTap: () {
                                _amountController.text =
                                    floor(model.withdrawForm.balance.quantity, 4);
                              },
                            )
                          ],
                        ),
                          ...
                          Builder(
                            builder: (context) {
                              if (model.verifyAddressResponseModel != null &&
                                  !model.verifyAddressResponseModel.valid) {
                                return Text("地址不正确", style: StyleFactory.errorMessageText);
                              } else if (model.withdrawForm.totalAmount.amount >
                                  model.withdrawForm.balance.quantity) {
                                return Text("余额不足", style: StyleFactory.errorMessageText);
                              } else if (model.withdrawForm.totalAmount.amount <
                                  model.gatewayAssetResponseModel.minWithdraw) {
                                return Text("提现小于最小提现额", style: StyleFactory.errorMessageText);
                              }
                              return Container();
                            },
                          )
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        ...
                        Text("${model.gatewayAssetResponseModel.withdrawFee} USDT",
                    ...
                    ButtonTheme(
                      ...
                      child: WidgetFactory.button(
                          data: "提现",
                          color: model.isHide && model.verifyAddressResponseModel != null
                              ? Palette.redOrange
                              : Palette.subTitleColor,
                          onPressed: model.isHide && model.verifyAddressResponseModel != null
                              ? () async {
                                  if (model.withdrawForm.cybBalance.quantity <
                                    ...
                                    model.withdrawForm.address = _addressEditController.text;
